...
int menuOnly()
{
	int escolha;
	system("cls");
	printf("!!!Observacao:Apos inserir uma nova ordem de servico,reinicie o aplicativo saindo pela opcao 0 para que a Ordem seja \nMugravada corretamente!!!\n");
...
char resposta;
FILE *arquivo;
arquivo=fopen("Ordem Serviço.txt","a");
...
void Cadastrar()
{
	...
	fflush(stdin);
	gets(adm.senha);
	printf("\nInforme o tipo de usuario<1-Tecnico/0-Administrador>: ");
	scanf("%i", &adm.perfil);
	fwrite(&adm, sizeof(adm),1,arquivo);
	fclose(arquivo);
}
...
void consulta()
{
	...
	printf("!Consulta!");
	FILE *ponteiro;
	ponteiro= fopen("Ordem Serviço.txt", "r");
...
void listar()
{
	...
	printf("!Listamento!\n");
	FILE *pontarq;
	pontarq=fopen("Ordem Serviço.txt", "r");
	if(pontarq == NULL)
	{
...
int main()
{
	...
	if((arquivo=fopen("Usuario.txt","r"))==NULL)
	...
	else
		{
			char consultaL[20];
			char consultaS[20];
			printf("!Por favor,insira seu login e senha para ter acesso ao progama!\n!Caso a senha ou usuario nao coincidam,o progama sera fechado!\n");
			printf("\n()Informe o login: ");
			fflush(stdin);
			gets (consultaL);
			printf("\n()Informe a senha: ");
